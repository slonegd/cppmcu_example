...
// заворачиваем это всё в отдельный namespace, потому что таких определений может быть много
// и имена будут пересекаться
namespace SysTick_ns { // имя SysTick занято под макрос CMSIS
...
// определим структуру переферии с помощью определённых ранее битовых полей
struct SysTick_t {
   __IOM SysTick_ns::CTRLbits  CTRL;  // Offset: 0x000 (R/W)  SysTick Control and Status Register
   __IOM uint32_t              LOAD;  // Offset: 0x004 (R/W)  SysTick Reload Value Register
   __IOM uint32_t              VAL;   // Offset: 0x008 (R/W)  SysTick Current Value Register
   __IM  SysTick_ns::CALIBbits CALIB; // Offset: 0x00C (R/ )  SysTick Calibration Register
...
struct template_SysTick {
   // как бы перебрасываем перечисление из неймспейса в структуру
   using Clock = SysTick_ns::Clock;
...
// в действительности данная структура SysTick_t работать не будет,
...
// как оформить, чтобы компилятор всегда генерировал ассембллерную команду
// обращения к регистру, как к двойному слову будет рассмотрено позже 
...
// и переписать pin.h используя её
