...
// Перенесём ListIterator в List, чтобы не засорять именами
...
// иногда компилятору надо подсказывать где тип, когда используется что-то шаблонное
// обычно сам компилятор и выдаёт ошибку
// error: need 'typename' before 'List<T>::Iterator' because 'List<T>' is a dependent scope
// поэтому добаляем typename
template<class T>
