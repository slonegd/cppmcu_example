...
enum class Periph {PA, PB, PC, PD, PF};
...
public:
   // конструктор по умолчанию нужен для тестирования 
   RCC() = default;

   // фабрика, чтобы получить правильный объект класса
   static auto& make() { return *reinterpret_cast<RCC*>(RCC_BASE); }

   // этот метод вызывается только приконструировании, потому делаем его шаблонным
   // в качестве параметра заведём специальное перечисление Periph
   template<Periph p> void clockEnable()
   {
      if      constexpr (p == Periph::PA) AHBENR.IOPAEN = true;
      else if constexpr (p == Periph::PB) AHBENR.IOPBEN = true;
      else if constexpr (p == Periph::PC) AHBENR.IOPCEN = true;
      else if constexpr (p == Periph::PD) AHBENR.IOPDEN = true;
      else if constexpr (p == Periph::PF) AHBENR.IOPFEN = true;
   }
};
