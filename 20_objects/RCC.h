...
public:
   RCC() = default;
   static inline auto& make() { return *reinterpret_cast<RCC*>(RCC_BASE); }
   inline void clockEnable (void* p)
   {
      auto adr = reinterpret_cast<size_t>(p);
      switch (adr) {
         case GPIOA_BASE: AHBENR.IOPAEN = true;
         case GPIOB_BASE: AHBENR.IOPBEN = true;
         case GPIOC_BASE: AHBENR.IOPCEN = true;
         case GPIOD_BASE: AHBENR.IOPDEN = true;
         case GPIOF_BASE: AHBENR.IOPFEN = true;
      };
   }
