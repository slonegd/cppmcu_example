...
class CyclicTask {
	...
	// сразу инициируем переменной нулём, считать будем до time
	std::size_t time_count {time};
public:
	...
	// решение есть (полиморфизм, наболюдатель), будет рассмотрено далее 
...
public:
	void tick()  { time_count++; }
	// переопределение оператора, просто заменили названия функции, остальное то же
...
void process()
{
   ...
   using GreenLed = PC9;
	...
	while(1)
	{
		// теперь task можно использовать как bool переменную
		if (cyclic_task) {
			BlueLed::toggle();
			GreenLed::toggle();
		}
...
// приведение к bool объектов класса CyclicTask семантически неверно ("если задача?")
...
void process()
{
	...
	while(1)
	{
		// теперь task - функциональный объект, который принимает другую функцию
...
void process()
{
	...
	while(1)
	{
		// теперь task - функциональный объект, который принимает безымянную
		// функцию - лямбду
		cyclic_task ([](){
			BlueLed::toggle();
			GreenLed::toggle();				
...
public:
	...
	// теперь функцию можно вызывать автоматически
	void tick()
	{
...
// для линковки с си кодом extern "C"
...
int main(void)
{
   // iter01::process(); // 568 байт
	...
	// iter05::process(); // 904 байт - не ссылаемся на глобальный объект-функцию
