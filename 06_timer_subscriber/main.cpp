/**
 * 06 программный таймер, как подписчик
 **/
...
// переписываем пример 02 используя паттерн подписчик
// всё, что касается двунаправленного списка перенёс в файд list.h
// классами из 04 воспользоваться не получится, так как нет std::list
...
// отличается от подписчика из 04 тем, что сам будет хранить указатели
...
struct TickUpdater : private List<TickSubscriber>
{
   ...
   // в издателе 04 тут был автоцикл, чтобы его реализовать
